...
def simple_region_growing(img, seed, threshold=1):
    ...
    #parameters
    mean_intensity_of_the_region = float(img[seed[1], seed[0]])
    size_of_region = 1
    ...
    while(distance_between_avg_intensity_of_region_and_current_neighbor_intensity<threshold and size_of_region<orig_image_area):
        ...
        for j in range(len(neighbor_orientation)):
            ...
            if (is_in_img and (region[neighbor_coordinates[1], neighbor_coordinates[0]]==0)):   ## Why 1,0 and not 0,1? is it an opencv thingy?
                contour_of_segmented_region.append(neighbor_coordinates)
                region_contour_intensity_values.append(img[neighbor_coordinates[1], neighbor_coordinates[0]] )
                region[neighbor_coordinates[1], neighbor_coordinates[0]] = 150

        #add the nearest pixel of the contour in it
        distance_between_avg_intensity_of_region_and_current_neighbor_intensity = abs(int(numpy.mean(region_contour_intensity_values)) - mean_intensity_of_the_region)
